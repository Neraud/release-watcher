...
def main(config: str):
    ...
    if global_config.core.run_mode == 'once':
        ...
        while True:
            _run_once(watcher_runner, outputs)
            logger.debug('Sleeping %d seconds ...', global_config.core.sleep_duration)
            time.sleep(global_config.core.sleep_duration)


def _create_watchers(global_config: GlobalConfig) -> Sequence[watcher_manager.Watcher]:
    ...
    for source_conf in global_config.sources:
        ...
        watchers_for_source = source_type.create_source(
            global_config.common, source_conf
        ).read_watchers()

        logger.debug(' -> %d watchers for %s', len(watchers_for_source), source_conf.name)
        for watcher_config in watchers_for_source:
            watcher_type = watcher_manager.get_watcher_type(watcher_config.watcher_type_name)
            watcher = watcher_type.create_watcher(watcher_config)
            watchers.append(watcher)
    return watchers


def _create_ouputs(outputs_conf: output_manager.OutputConfig) -> Sequence[output_manager.Output]:
    logger.info('Creating outputs')

    return [
        output_manager.get_output_type(output_conf.name).create_output(output_conf)
        for output_conf in outputs_conf
    ]


def _run_once(watcher_runner: WatcherRunner, outputs: Sequence[output_manager.Output]):
